...
void move_robot(float leftSpeed, float rightSpeed) {
  ...
  // Set left LED color to speed
  float speed = leftSpeed + rightSpeed;
  //set_left_led_color(CHSV((uint8_t)map(speed, -512, 512, -96, 96), 255, 255));
}
...
void forward_ramp() {
  // slowly increase speed to speed_multiplier
  float end_speed_mult = speed_multiplier;

  for(float mult = 0.2; mult < end_speed_mult; mult += 0.1) {
    set_speed(mult);
    forward();
